---
layout: page
sidenav: true
type: training
title: Accessible Meetings
permalink: training/online-course/accessible-meetings/
description: [pending]
audience: 
- Communications Specialists
- Federal Employees
- Section 508 Program Managers
topic: 
- Create
- Other
- Training
subtopic: 
- Audio, Video and Multimedia
- Best Practices
- Webinar
resource-type: 
- Training
format: HTML
created: 2026-04-08
updated: 
---

<ul class="usa-card-group">
  <li class="usa-card usa-card--flag flex-1 usa-card--media-right">
    <div class="usa-card__container">
      <div class="usa-card__header">
        <h4 class="usa-card__heading">Course Description</h4>
      </div>
      <div class="usa-card__media usa-card__media--inset">
        <div class="usa-card__img">
          <img src="{{site.baseurl}}/assets/images/thumbnails/training-landing-meetings.jpg" alt="" class="radius-lg" />
        </div>
      </div>
      <div class="usa-card__body">
        <p>This training covers various techniques to create accessible meetings. Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat. Duis aute irure dolor in reprehenderit in voluptate velit esse cillum dolore eu fugiat nulla pariatur. Excepteur sint occaecat cupidatat non proident, sunt in culpa qui officia deserunt mollit anim id est laborum.</p>
      </div>
      <div class="usa-card__footer">
        <a href="{{site.baseurl}}/assets/online-training/microsoft-word-accessibility-best-practices/story.html" target="_blank" class="usa-button">Open Course</a>
      </div>
    </div>
  </li>
</ul>

## Course Details
**Audience:** 
* Communications Specialists
* Federal Employees
* Section 508 Program Managers

**Duration:** 60 minutes  
**Difficulty Level:** Beginner  
**Version:** 1.0 (Apr 2026)

## What Will I Learn in This Course?
In recent years, how agencies gather, collaborate, discuss, share, and learn has changed from largely in-person meetings and conference calls to virtual meetings where the audience participates remotely through voice and video from various work sites. Now, meetings are often virtual, or a hybrid of in-person and remote participants. For this reason, accessibility can and should be seamlessly integrated into your meeting.

But what types of meetings need to be accessible? How can you make your presentation materials accessible to all meeting attendees? In this course, you will learn how to make your meetings accessible to attendees, including people with disabilities in compliance with Section 508 and other disability rights laws.

After completing this training, you will be able to do the following:
<ol class="usa-icon-list tablet:grid-col">
  <li class="usa-icon-list__item">
      <div class="usa-icon-list__icon text-green"><svg class="usa-icon" aria-hidden="true" role="img"><use xlink:href="{{ site.baseurl }}/assets/images/sprite.svg#check_circle"></use></svg></div>
      <div class="usa-icon-list__content">Identify which accessibility requirements apply to each type of meeting.</div>
  </li>
  <li class="usa-icon-list__item">
      <div class="usa-icon-list__icon text-green"><svg class="usa-icon" aria-hidden="true" role="img"><use xlink:href="{{ site.baseurl }}/assets/images/sprite.svg#check_circle"></use></svg></div>
      <div class="usa-icon-list__content">Apply techniques to make meeting visuals, audio, content, and interactions accessible.</div>
  </li>
  <li class="usa-icon-list__item">
      <div class="usa-icon-list__icon text-green"><svg class="usa-icon" aria-hidden="true" role="img"><use xlink:href="{{ site.baseurl }}/assets/images/sprite.svg#check_circle"></use></svg></div>
      <div class="usa-icon-list__content">Prepare and facilitate a meeting that meets all Section 508 requirements.</div>
  </li>
</ol>
 
## Certificate and Continuing Learning Points (CLPs)
Demonstrate your skills and stand out in your field with a certificate that proves you’ve completed this course. There is only one type of certificate offered for this course.
<ol class="usa-icon-list tablet:grid-col">
  <li class="usa-icon-list__item">
      <div class="usa-icon-list__icon text-green"><svg class="usa-icon" aria-hidden="true" role="img"><use xlink:href="{{ site.baseurl }}/assets/images/sprite.svg#check_circle"></use></svg></div>
      <div class="usa-icon-list__content">By selecting "Open Course" you will earn a downloadable PDF certificate upon completion.</div>
  </li>
  <li class="usa-icon-list__item">
      <div class="usa-icon-list__icon text-red"><svg class="usa-icon" aria-hidden="true" role="img"><use xlink:href="{{ site.baseurl }}/assets/images/sprite.svg#cancel"></use></svg></div>
      <div class="usa-icon-list__content">This course is <strong>not</strong> offered with continuing learning points (CLP).</div>
  </li>  
</ol>

## Key Terms and References 
* Law & Policy 
  * [Section 504 of the Rehabilitation Act](https://www.dol.gov/agencies/oasam/centers-offices/civil-rights-center/statutes/section-504-rehabilitation-act-of-1973)
  * [Architectural Barriers Act (ABA)](https://www.access-board.gov/about/law/aba.html)
  * [Americans with Disabilities Act (ADA)](https://www.ada.gov/law-and-regs/ada/)
* 
* [Authoring Meaningful Alternative Text](https://www.section508.gov/create/alternative-text/)
* [Create Accessible Meetings](https://www.section508.gov/create/accessible-meetings/)
* [Creating Accessible Email Messages](https://www.section508.gov/create/email-messages/)
* [Buy Accessible Products and Services](https://www.section508.gov/buy/)
* [Functional Performance Criteria](https://www.access-board.gov/ict/#302-functional-performance-criteria)
* [Video and Other Synchronized Media](https://www.section508.gov/create/synchronized-media/)
* [Section 508 Program Manager](https://www.section508.gov/tools/program-manager-listing/)
* <a href="https://www.access-board.gov/">U.S. Access Board</a>
* In-Course Resources
  * [Create Accessible Digital Products](https://www.section508.gov/create)
  * [Universal Design and Accessibility](https://www.section508.gov/create/universal-design)
  * [How People with Disabilities Used the Web](https://www.w3.org/WAI/people-use-web/) (W3C WCAG)
  * [Making Meetings Accessible](https://www.cdc.gov/ncbddd/hearingloss/transcripts/Making-Meetings-Accessible.pdf) (CDC.gov)
  * [NYC Accessible Virtual Meetings Guide](https://www1.nyc.gov/assets/mopd/downloads/pdf/virtual-meetings-accessibility-guide.pdf) (NYC.gov)
  * [How to Make Your Presentations and Meetings Accessible to All | Web Accessibility Initiative (WAI) | W3C](https://www.w3.org/WAI/teach-advocate/accessible-presentations/)
  * [Accessibility of Remote Meetings | W3C](https://www.w3.org/TR/remote-meetings/#creating-accessible-content-for-remote-meetings)
* Meeting Platform Accessibility Guides
    * [Adobe Connect Accessibility Features](https://helpx.adobe.com/adobe-connect/using/accessibility-features.html)
    * [Keyboard shortcuts for Adobe Connect Classic](https://helpx.adobe.com/adobe-connect/using/user-guide.html/adobe-connect/using/connect-keyboard-shortcut.ug.html)
    * [Downloading and Running a Closed-captioning Pod in an Adobe Connect Meeting](https://blogs.connectusers.com/connectsupport/downloading-and-running-a-closed-captioning-pod-in-a-connect-meeting/)
    * [Cisco WebEx Accessibility](https://www.webex.com/accessibility.html)
    * [Webex Meetings and Webex Events Accessibility Features](https://help.webex.com/en-us/84har3/Webex-Meetings-and-Webex-Events-Accessibility-Features)
    * [How Do I Enable Closed Captions in Webex?](https://help.webex.com/en-us/WBX47352/How-Do-I-Enable-Closed-Captions)
    * [Google Meet Accessibility](https://support.google.com/meet/answer/7313544?hl=en)
    * [GoToMeeting Accessibility Features](https://support.goto.com/meeting/help/what-accessbility-features-are-available-in-gotomeeting)
    * [Use Keyboard Shortcuts from the Desktop App (Windows Only)](https://support.goto.com/meeting)
    * [Microsoft Teams Accessibility](https://support.office.com/en-us/article/accessibility-overview-of-microsoft-teams-2d4009e7-1300-4766-87e8-7a217496c3d5)
    * [Accessibility overview of Microsoft Teams](https://support.office.com/en-us/article/accessibility-overview-of-microsoft-teams-2d4009e7-1300-4766-87e8-7a217496c3d5)
    * [Keyboard shortcuts for Microsoft Teams](https://support.microsoft.com/en-us/office/keyboard-shortcuts-for-microsoft-teams-2e8e2a70-e8d8-4a19-949b-4c36dd5292d2)
    * [Accessible and Inclusive Telework Practices Using Microsoft Teams](https://www.hud.gov/sites/dfiles/OCIO/documents/MSTeams_covid19.pdf) - Guidance from HUD
    * [5 Tips on Holding Accessible Microsoft Team Meetings](https://www.hud.gov/sites/dfiles/OCIO/documents/5Tips_covid19.pdf) - Guidance from HUD
    * [Zoom Accessibility](https://zoom.us/accessibility)
    * [Hot keys and keyboard shortcuts for Zoom](https://support.zoom.us/hc/en-us/articles/205683899-Hot-Keys-and-Keyboard-for-Zoom)
    * [Getting started with closed captioning in Zoom](https://support.zoom.us/hc/en-us/articles/207279736-Getting-started-with-closed-captioning)
    * [Join a Test Zoom Meeting](https://zoom.us/test)
* Vocabulary
  * [CART (Communication Access Realtime Translation or CART)](https://www.section508.gov/content/glossary/#cart)&mdash;real-time captioning services 
  * [American Sign Language (ASL) interpreter]()

<div class="usa-accordion usa-accordion--bordered">
  <h2 class="usa-accordion__heading"><button type="button" class="usa-accordion__button" aria-expanded="false" aria-controls="change-log">Change Log</button>
  </h2>
  <div id="change-log" class="usa-accordion__content usa-prose">
    <ul>
      <li class="usa-icon-list__item">
        <div class="usa-icon-list__icon text-orange"><svg class="usa-icon" aria-hidden="true" role="img"><use xlink:href="{{ site.baseurl }}/assets/images/sprite.svg#event"></use></svg></div>
        <div class="usa-icon-list__content"><strong>April 2026 - v1.0 | Release:</strong> Our initial release of the <em>Accessible Meetings</em> course with core lessons, practice quizzes, and foundational learning.</div>
      </li>
    </ul>
  </div>
</div>