...
For example, the - (dash) is a heavily used key for many different purposes including as a ˗ (minus) symbol in equations. Assistive technologies, especially screen readers, will identify this symbol to the end user as either **dash** or **minus**. It does make a difference on what you ‘type’ as to which description the end user will receive. This is determined by the Unicode designation that is assigned to the character.  

There is an alternative for simple mathematical modifiers that is equivalent to the Unicode designation. Instead of using the number row of keys along the top of a standard keyboard, use the **Numeric Keypad** (Figure 1). When Num Lock (Clear on a mac keyboard) is activated, the numeric keyboard becomes active as a calculator. Instructions are sent to the computer to use the correct mathematical operator based on the user’s input from the numeric keypad. However, this is very limited as you only have 5 basic operators available to you; ˗ (minus), + (plus), × (multiplication), ÷ (division represented as a / forward slash), and = (equals).  
