---
layout: wide_events
title: A Systematic Approach to PDF Accessibility | Webinar
permalink: /event/best-practices-webinar-may2026/
description: Master PDF accessibility—from basics to advanced fixes—in this practical webinar. Learn how people with disabilities use PDFs, tackle common and complex issues, go beyond Acrobat’s checker, and speed remediation workflows with expert tools.
start-date: #only fill in start date if the events spans multiple days
end-date: 2026-05-19
event-registration: https://www.accessibilityonline.org/ADA-Audio/session?id=111220
audience: 
- Content Creators
- Section 508 Testers
- Specialists & SMEs
- Web Content Managers
topic: 
- Create
- Testing
- Training
subtopic: 
- Documents
- Guidance
- Training
resource-type: 
- Training
format: HTML
created: 2026-04-22
updated: 
---

Whether you are new to PDF accessibility or struggling to remediate more advanced issues, this webinar is here to help. Join our presenters to become more aware of how people with disabilities use PDFs, discuss the most common and more irregular document accessibility issues, learn how to go beyond the Acrobat Accessibility Checker, and gain insight into the go-to tools they use to speed up remediation workflows. Registrants will be able to submit questions in advance during the registration process.

### Event Details
**DATE:** Tuesday, March 31, 2026   
**TIME:** 1:00 PM - 2:30 PM ET  
**WHERE:** Virtual webinar

### Registration
{% include event-registration.html %}